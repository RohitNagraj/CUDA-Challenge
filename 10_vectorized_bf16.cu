...
__global__ void transform(__nv_bfloat16 *arr, __nv_bfloat16 *output, float *sum)
{
    ...
    if (i < N)
        *reinterpret_cast<float2 *>(&cache[threadIdx.x * 4]) = *reinterpret_cast<float2 *>(&arr[i]);
    ...
    if (i < N)
    {
        ...
        // Store 4 bfloat16s
        *reinterpret_cast<float2 *>(&output[i]) = *((float2 *)&output_vec);
        
        // Now accumulate for sum (when idx % 4 == 1 and result > 0.5)
        if (__bfloat162float(output_vec[1]) > 0.5f)
            thread_sum += __bfloat162float(output_vec[0]);
    }
